...
const ShowMembers = () => {
  ...
  return (
    <div className='h-screen over-flow-y z-0 relative'>
      ...
      <div className='absolute z-10 mt-0 w-10/12'>{showDelete ? <DeleteMember setShowDelete={setShowDelete} /> : ''}</div>
      ...
      <table className='table-auto bg-blue-800 border border-3 md:w-full'>
        ...
        <tbody>
          {
            currentMembers.map((member, index) => (
              <tr className='border md:text-sm text-xs text-neutral-200 font-light ' key={member.id}>
                ...
                className='border-r px-1'><svg xmlns="http://www.w3.org/2000/svg" fill="red" viewBox="0 0 24 24" stroke-width="1.5" stroke="white" class="w-6 h-6">
       ...
       {members.length >= 10?    <tr key="" className='border'>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
